...
CREATE OR REPLACE TABLE dbx_joshy_demo.processed.cg_coin_historical_chart_data
...
WITH parsed_wrapper AS (
  ...
  WHERE file_name LIKE '%market_chart%'  -- Filter for market_chart files only
...
--   SUM(CASE WHEN price IS NOT NULL THEN 1 ELSE 0 END) as records_with_price,
--   SUM(CASE WHEN market_cap IS NOT NULL THEN 1 ELSE 0 END) as records_with_market_cap,
--   SUM(CASE WHEN total_volume IS NOT NULL THEN 1 ELSE 0 END) as records_with_volume
